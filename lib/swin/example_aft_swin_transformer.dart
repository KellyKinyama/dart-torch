...
void main() {
  ...
  const List<int> depths = [
    2,
    2,
    6,
    2
  ]; // Number of Swin Transformer blocks in each of 4 stages
  const List<int> numHeads = [
    3,
    6,
    12,
    24
  ...
  print(
      "  Initial Patch Grid: $initialNumPatchesH x $initialNumPatchesW ($totalInitialPatches patches)");
  ...
  print(
      "SwinTransformer initialized. Total parameters: ${model.parameters().length}");
  ...
  final List<ValueVector> dummyImagePatches = List.generate(
    ...
    (patchIdx) => ValueVector(
      List.generate(
        ...
        (valIdx) =>
            Value(math.Random().nextDouble() * 0.1), // Small random values
      ),
    ),
  );
  print(
      "Dummy image input created: ${dummyImagePatches.length} patches, each with ${dummyImagePatches[0].values.length} features.");
  ...
  try {
    ...
    if (outputFeatures.isNotEmpty) {
      print(
          "  Feature dimension per token: ${outputFeatures[0].values.length}");
      print(
          "  Expected final patch grid size: ${imageSize ~/ (patchSize * math.pow(2, depths.length - 1))} x ${imageSize ~/ (patchSize * math.pow(2, depths.length - 1))} = ${outputFeatures.length}");
    ...
    print(
        "Please ensure all dependencies (Value, ValueVector, Module, Layer, etc.) are correctly implemented and imported.");
  }
