...
class SelfAttention extends Module {
  ...
  SelfAttention(int embedSize, this.headSize, {this.masked = false})
      : keys = Layer.fromNeurons(embedSize, headSize),
        queries = Layer.fromNeurons(embedSize, headSize),
        values = Layer.fromNeurons(embedSize, headSize);

  List<ValueVector> forward(List<ValueVector> x) {
    final T = x.length;
    final k = x.map((v) => keys.forward(v)).toList();
    final q = x.map((v) => queries.forward(v)).toList();
    final v = x.map((v) => values.forward(v)).toList();

    var wei = List.generate(T, (i) {
      final row = List.generate(T, (j) => q[i].dot(k[j]));
      return ValueVector(row) * Value(1.0 / math.sqrt(headSize));
    });
    ...
    if (masked) {
      wei = List.generate(T, (i) {
        final newVals = wei[i].values.asMap().entries.map((entry) {
          ...
          if (j > i) return Value(double.negativeInfinity, {val}, 'mask');
          return val;
        }).toList();
    ...
    final pAttn = wei.map((row) => row.softmax()).toList();

    final out = List.generate(T, (i) {
      var posOut = ValueVector(List.filled(headSize, Value(0.0)));
      for (int j = 0; j < T; j++) {
...
void main() {
  ...
  final firstQueryWeight = selfAttention.queries.neurons[0].w.values[0];
