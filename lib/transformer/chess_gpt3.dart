...
class ChessGptFullGame {
  ...
  void train(List<String> gameMoves) {
    final game = Game();
  ...
  String predictNextMove(Game game) {
  ...
  /// Generates a full game sequence by repeatedly predicting the next move
  /// and playing it on the board.
  Future<List<String>> generateGameSequence({int maxMoves = 100}) async {
    final game = Game();
    final List<String> generatedMoves = [];

    // The token sequence starts with a special <start> token
    ...
    for (int i = 0; i < maxMoves; i++) {
      ...
      // Pad the input sequence to the block size for the decoder
      ...
      final List<ValueVector> logits =
          await decoder.forward(paddedInput, encoderOutput);
      final ValueVector lastTokenLogits = logits.last;
      ...
      // Sample a move from the distribution (or greedily take the best)
      // This implementation uses greedy selection for simplicity.
      double maxProb = -1.0;
      int predictedNextToken = -1;
      for (int j = 0; j < probabilities.values.length; j++) {
        if (probabilities.values[j].data > maxProb) {
      ...
      // Make the predicted move on the game board
      ...
      if (moveObject != null) {
        ...
        // Keep the input sequence from growing beyond the block size
        if (inputSequence.length > blockSize) {
          inputSequence.removeAt(0);
        }
      } else {
        // If the model suggests an illegal move (due to a bug or poor training), break.
        break;
      }
    }
