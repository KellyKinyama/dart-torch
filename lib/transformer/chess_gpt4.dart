...
class ChessGptFullGame {
  ...
  Future<void> train(List<List<String>> gameLines) async {
    int trained = 0;
    for (List<String> gameMoves in gameLines) {
      print('Starting training on a sample game: ${++trained} ...');
      final game = Game();
      // Removed startTokenId from the initial sequence
  ...
  Future<String> predictNextMove(Game game) async {
    ...
    // Removed startTokenId from the input sequence
    final List<int> inputSequence = [...gameHistoryTokens];
