...
class ChessGptFullGame {
  ...
  static ChessGptFullGame create() {
    ...
    final (moveStoi, moveItos) = _generateChessVocab();
  ...
  //   Set<String> uniqueMoves = {};
  //   for (final sequence in gameSequences) {
  //     uniqueMoves.addAll(sequence);
  //   }
  //   final List<String> sortedUniqueMoves = uniqueMoves.toList()..sort();
  //   final Map<String, int> stoi = {
  //     '<pad>': 0,
  //     '<start>': 1,
  //     '<end>': 2,
  ...
  static (Map<String, int>, Map<int, String>) _generateChessVocab() {
    ...
    // Generate all moves up to a certain depth (e.g., depth 2 or 3 is sufficient for a large vocab)
...
// Corrected function to tokenize a board state from a FEN string
  ...
  //     // int value = game.variant.pieces[type].value;
  //     // }
  ...
  final Map<int, String> pieceTypeNames = {
    1: 'pawn',
    2: 'knight',
    3: 'bishop',
    4: 'rook',
  ...
  List<int> _tokenizeBoardStateFromFen(String fen, Map<String, int> boardStoi) {
    ...
    for (int i = 0; i < size.numIndices; i++) {
      ...
      final squareValue = board[i]; // The square is an integer
  ...
  Future<void> train(List<String> gameMoves) async {
    ...
    // Encode the board state from the final FEN string
